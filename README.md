...
In this implementation, I've modeled the linear function and the non-linear activations are separate layers in the network. The linear layers are modeled as:
...
So now we can create a neural network by chaining together layers; alternating linear layers with non-linear activation layers. But how do we train to produce the right answer? That requires:
...
The layer then needs to produce a "dy" value for the previous layer (which is the same thing as the "dx" value for this layer - the gradient of it's input with respect to it's dy).
